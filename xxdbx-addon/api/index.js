...
async function fetchPage(url) {
  const res = await fetch(url, { headers: { 'User-Agent': UA, 'Referer': 'https://xxdbx.com/' }, timeout: 15000 });
...
// ─── Manifest ──────────────────────────────────────────────
// V11: Exact same as V6 (which WORKED) — stream types ONLY ['movie']
// V10 broke navigation by adding 'channel' to stream types
...
const MANIFEST = {
  ...
  resources: [
    ...
    {
      name: 'stream',
      types: ['movie'],           // ← V6 had ONLY 'movie' — this is what worked!
      idPrefixes: ['video_']
    }
  ],
  types: ['channel', 'movie'],
...
app.get('/play/:videoId/:qualityFile', async (req, res) => {
  ...
  try {
    ...
    const fetchHeaders = { 'User-Agent': UA, 'Referer': 'https://xxdbx.com/' };
    if (range) {
...
if (require.main === module) {
  ...
  app.listen(PORT, () => {
    console.log(`XXDBX Stremio addon V11 running on http://localhost:${PORT}`);
